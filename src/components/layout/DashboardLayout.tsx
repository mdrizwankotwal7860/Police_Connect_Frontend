      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-200/50 bg-white/80 backdrop-blur-md shadow-sm">
        <div className="container mx-auto flex h-20 lg:h-24 items-center justify-between px-2 lg:px-8">
          ...
          <div className="flex-1 flex items-center justify-start gap-1 lg:gap-4">
            <Button variant="ghost" size="icon" className="lg:hidden shrink-0 h-9 w-9 hover:bg-slate-100 rounded-md text-slate-900" onClick={() => setSidebarOpen(true)} aria-label="Open navigation menu">
          ...
          <div className="flex-1 flex items-center justify-end gap-2 lg:gap-4">
            <div className="flex flex-col items-end mr-1 lg:mr-2">
              <span className="text-xs sm:text-sm font-semibold text-slate-900 whitespace-nowrap max-w-[80px] sm:max-w-none overflow-hidden text-ellipsis">{user.name}</span>
              <span className="text-[10px] sm:text-xs text-slate-500 capitalize">{user.role.replace('_', ' ')}</span>
            </div>
            
            {/* Right Gun */}
            <div className="relative h-7 w-7 sm:h-10 sm:w-10 lg:h-14 lg:w-14 rounded-full overflow-hidden shadow-sm shrink-0 border border-slate-200 bg-white">
